...
import setup
import log
from auto_labeling.config import get_config, get_work_directory
...
class Model(QObject):
    ...
    def allow_migrate_data():
        ...
        try:
            ...
            log.error(f"An error occurred during data migration: {str(e)}")
            return False
    ...
    def get_model_abs_path(self, model_config, model_path_field_name):
        ...
        if os.path.exists(model_abs_path):
            ...
            if is_known_type:
                log.info(f"Validating model integrity: {filename}")
            ...
            if is_valid:
                log.info(f"Model file is valid: {model_abs_path}")
                return model_abs_path
            else:
                log.warn(
                ...
                try:
                    ...
                    log.error(f"Could not delete corrupted file: {str(e2)}")
        ...
        log.info(f"Downloading {download_url} to {model_abs_path}")
        try:
    ...
    def load_image_from_filename(filename):
        ...
        if QFile.exists(label_file) and LabelFile.is_label_file(label_file):
            try:
                ...
                log.error("Error reading {}: {}".format(label_file, e))
                return None, None
        ...
        if image.isNull():
            log.error("Error reading {}".format(filename))
        return image
