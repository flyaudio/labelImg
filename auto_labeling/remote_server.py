...
class RemoteServer(Model):
    ...
    def get_available_models(self):
        ...
        try:
            ...
            log.error(f"Failed to fetch available models: {e}")
            return {}
    ...
    def predict_shapes(
        ...
        if batch_mode == "video" and text_prompt:
            ...
            if run_tracker:
                log.info("Starting video propagation after point prompt...")
                return self._handle_video_propagation()
            return result
        elif batch_mode == "video" and run_tracker:
            log.info("Starting video propagation...")
            return self._handle_video_propagation()
        ...
        if image_path and os.path.exists(image_path):
            ...
            img_data_uri = f"data:{MIME_TYPE.get(ext, 'image/jpeg')};base64,{img_base64}"
        else:
            try:
                cv_image = qt.to_rgb_cv_img(image, image_path)
            except Exception as e:
                log.warning(f"Could not process image: {e}")
                return AutoLabelingResult([], replace=self.replace)
        ...
        try:
            ...
            response.raise_for_status()
            result = response.json()
            log.debug(f"Remote server prediction result: {result}")
            ...
            log.error(f"Remote server err: {e}")
            self.on_message(f"Remote server err: {e}")
            return AutoLabelingResult([], replace=self.replace)
    ...
    def _handle_video_prompt(self, image_path, text_prompt):
        ...
        try:
            ...
            if (
                ...
                log.info("Image list changed, resetting video session")
            ...
            log.error(f"Video prompt error: {e}")
    ...
    def _handle_video_point_prompt(self, image_path):
        ...
        try:
            ...
            if not point_coords:
                log.warning("No valid point prompts provided")
                return AutoLabelingResult([], replace=False)
            ...
            log.error(f"Video point prompt error: {e}")
    ...
    def _handle_video_propagation(self):
        ...
        if not self.video_session_id:
            log.warning("No video session initialized")
            return AutoLabelingResult([], replace=False)
        ...
        try:
            ...
            for line in response.iter_lines(decode_unicode=True):
                if widget and getattr(widget, "cancel_processing", False):
                    log.info("Propagation cancelled by user")
                    cancelled = True
                    response.close()
                    break
                ...
                if event_type == "started":
                    ...
                    log.error(f"Propagation error: {error_msg}")
            ...
            if not results:
                log.warning("No results received from propagation")
                return AutoLabelingResult([], replace=False)

            if not widget or not image_list:
                log.warning("Widget or image list not available")
                return AutoLabelingResult([], replace=False)
            ...
            log.info(f"Saved results for {saved_count} frames")
            return AutoLabelingResult([], replace=False)

        except requests.exceptions.RequestException as e:
            log.error(f"Video propagation request error: {e}")
            self.on_message(f"Video propagation error: {e}")
            self._reset_video_session()
            self.label, self.group_id = None, None
            return AutoLabelingResult([], replace=False)
        except Exception as e:
            log.error(f"Video propagation error: {e}", exc_info=True)
            self.on_message(f"Video propagation error: {e}")
            self._reset_video_session()
            self.label, self.group_id = None, None
            return AutoLabelingResult([], replace=False)
    ...
    def _reset_video_session(self):
        ...
        if self.video_session_id:
            try:
                ...
                log.debug(f"Cleaned up session {self.video_session_id}")
            except Exception as e:
                log.warning(f"Failed to cleanup session: {e}")
