...
import setup
import log
from auto_labeling.config import get_config
import auto_labeling.config as CONFIG
from auto_labeling.model_manager import ModelManager
# from auto_labeling.types import AutoLabelingMode
...
from auto_labeling.style import (
    get_lineedit_style,
...
from auto_labeling.remote_server_dialog import (
    RemoteServerDialog,
)
from auto_labeling.searchable_model_dropdown import (
#     load_json,
#     save_json,
    _get_models_config_path,
    SearchableModelDropdownPopup,
)
from libs.stringBundle import get_str as tr
import libs.utils
...
class AutoLabelingWidget(QWidget):
    ...
    auto_segmentation_requested = pyqtSignal()  #用户点击「运行」按钮，请求执行自动分割推理
    auto_segmentation_disabled = pyqtSignal()  #用户关闭 / 停用了自动分割功能
    # auto_labeling_mode_changed = pyqtSignal(AutoLabelingMode) #用户切换了标注模式（比如点选模式、矩形框模式）
    clear_auto_labeling_action_requested = pyqtSignal() #用户点击「清除」按钮，请求清空当前自动标注结果
    finish_auto_labeling_object_action_requested = pyqtSignal()
    cache_auto_label_changed = pyqtSignal() #自动标注的缓存开关状态发生了变化
    auto_decode_mode_changed = pyqtSignal(bool)
    cropping_mode_changed = pyqtSignal(bool) #用户切换了「裁剪模式」的开关
    clear_auto_decode_requested = pyqtSignal()
    mask_fineness_changed = pyqtSignal(float) #用户拖动了「蒙版精细度」滑块
    ...
    def __init__(self, parent):
        ...
        scroll_bar.rangeChanged.connect(self._update_model_selection_scroll_area_height)
        ...
        self.button_run.setText(tr("Run (i)"))
    ...
    #         # import config
    #         shortcuts = get_config().get("shortcuts", {})
    ...
    def init_model_data(self):
        ...
        # try:
        #     log.info("models config path:", CONFIG.get_models_config_path())
        #     local_model_data = libs.utils.load_json(CONFIG.get_models_config_path())["models_data"]
        ...
        for model_dict in model_list:
            ...
            if (provider_name in local_model_data
        ...
        sorted_model_data = sort_model_data(model_data)
            ...
            if config_path and config_path.startswith(":/"):
                try:
                    ...
                    if dialog.exec() == QDialog.DialogCode.Accepted:
                        ...
                        if new_url:
                            self.model_manager.update_model_config(config_path, "server_url", new_url)
                        self.model_manager.update_model_config(config_path, "api_key", new_api_key)
                        ...
                        # if new_url:
                        #     self.parent._config["remote_server_settings"]["server_url"] = new_url
                        # self.parent._config["remote_server_settings"]["api_key"] = new_api_key
                    else:
                        return
                except Exception as e:
                    log.error(f"Failed to process remote_server config: {e}")
                    return
    ...
    #     # Validate model status
        if model_name not in self.model_info:
            log.warn(f"Model '{model_name}' is not defined or no longer available. ")
    ...
    def run_prediction(self):
        ...
        if self.parent.filename is not None:
            ...
            self.model_manager.predict_shapes_threading(self.parent.image, self.parent.filename)
    ...
    def on_new_model_loaded(self, model_config):
        ...
        try:
            if self.model_manager.loaded_model_config["type"] in CONFIG._AUTO_LABELING_IOU_MODELS:
                if "iou_threshold" in self.model_manager.loaded_model_config:
                    ...
                    initial_iou_value = self.model_manager.loaded_model_config["nms_threshold"]
        ...
        try:
            if self.model_manager.loaded_model_config["type"] in CONFIG._AUTO_LABELING_CONF_MODELS:
                if "conf_threshold" in self.model_manager.loaded_model_config:
                    ...
                    initial_conf_value = self.model_manager.loaded_model_config["box_threshold"]
                elif "confidence_threshold" in self.model_manager.loaded_model_config:
                    initial_conf_value = self.model_manager.loaded_model_config["confidence_threshold"]
        ...
        # el
        if model_config.get("type") == "remote_server":
    ...
    def _update_model_selection_scroll_area_height(self, *_):
        update_model_selection_scroll_area_height(self.model_selection_scroll_area)
    ...
    def update_remote_server_mode_ui(self):
        ...
        if (
            not self.model_manager.loaded_model_config
            or self.model_manager.loaded_model_config.get("type") != "remote_server"
        ):
            return
        ...
        for model_id, model_info in available_models.items():
            ...
            self.remote_server_select_combobox.addItem(display_name, userData=model_id)
    ...
    def on_task_changed(self):
        ...
        log.info(f"Task changed - index: {current_index}, task_id: {task_id}")
        ...
        if task_info:
            ...
            log.warn(f"Task info not found for task_id: {task_id}")
    ...
    def update_task_mode_ui(self, available_tasks):
        ...
        for task in available_tasks:
            ...
            self.remote_task_select_combobox.addItem(task_name, userData=task_id)
            ...
            if widget:
                ...
                if widget_value is not None:
                    ...
                    if widget_name == "edit_conf":
                        ...
                        self._on_toggle_preserve_existing_annotations_toggled(widget_value)
    ...
    def _extract_shapes_for_recognition(self):
        ...
        for shape in self.parent.canvas.shapes:
            if shape.shape_type in ["rectangle", "rotation", "polygon"]:
                ...
                error_text = self.tr(
                    "err shape type. Only rectangle, rotation and polygon shapes are supported for detection boxes."
                )
...
def get_labeling_instruction():
    text_mode = tr("Mode:")
    text_shortcuts = tr("Shortcuts:")
    text_settings = tr("Settings")
    text_previous = tr("Previous")
    text_next = tr("Next")
    text_rectangle = tr("Rectangle")
    text_polygon = tr("Polygon")
    text_rotation = tr("Rotation")
    text_quadrilateral = tr("Quadrilateral")
    shortcuts = get_config().get("shortcuts", {})
    return (
        # f"<b>{text_mode}</b> {self.canvas.get_mode()} | "
        f"<b>{text_shortcuts}</b>"
        f" {text_previous}({shortcuts.get('open_prev')}),"
        f" {text_next}({shortcuts.get('open_next')}),"
        f" {text_rectangle}({shortcuts.get('create_rectangle')}),"
        f" {text_polygon}({shortcuts.get('create_polygon')}),"
        f" {text_rotation}({shortcuts.get('create_rotation')}),"
        f" {text_quadrilateral}({shortcuts.get('create_quadrilateral')}),"
        f" {text_settings}({shortcuts.get('open_settings')})"
    )


def sort_model_data(model_data: dict) -> collections.OrderedDict:
