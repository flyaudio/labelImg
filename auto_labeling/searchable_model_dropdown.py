...
import setup
from auto_labeling.config import get_work_directory
# from anylabeling.views.labeling.chatbot.config import *
import auto_labeling.config as CONFIG
from libs.utils import load_json, save_json
from auto_labeling.qt import new_icon, new_icon_path
from auto_labeling.theme import get_theme
...
class SearchBar(QLineEdit):
    def __init__(self, parent=None):
        ...
        t = get_theme()
        self.setPlaceholderText("Search models") #提示信息
        self.setFixedHeight(CONFIG.DEFAULT_FIXED_HEIGHT)
        ...
        self.search_icon.setPixmap(
            QIcon(new_icon("search", "svg")).pixmap(QSize(*(CONFIG.ICON_SIZE_SMALL)))
        )
        print("")
...
class ProviderSection(QFrame):
    def __init__(self, provider_name, parent=None):
        ...
        icon.setPixmap(
            QIcon(new_icon(icon_name, ext)).pixmap(QSize(*(CONFIG.ICON_SIZE_SMALL)))
        )
...
class ModelItem(QFrame):
    ...
    def __init__(
        ...
        self.name_label.setStyleSheet(
            f"font-size: {CONFIG.FONT_SIZE_SMALL}; color: {_t['text']}; background: transparent;"
        )
        ...
        self.check_icon.setFixedSize(QSize(*(CONFIG.ICON_SIZE_SMALL)))
        if self.is_selected:
            self.check_icon.setPixmap(
                QIcon(new_icon("check", "svg")).pixmap(QSize(*(CONFIG.ICON_SIZE_SMALL)))
            )
        ...
        self.star_icon.setFixedSize(*(CONFIG.ICON_SIZE_SMALL))
